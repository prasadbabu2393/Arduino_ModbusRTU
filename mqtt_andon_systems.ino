...
void setup() 
{
  ...
  preferences.begin("storage", false);  // same namespace[storage] for switchs and network, password
  String flash_flag  = preferences.getString("flash", "");  //  reading a flag

  if(flash_flag != "")
  {
    ssid = preferences.getString("ssid", "");   //ssid reading from flash memory
    password = preferences.getString("password", ""); //password reading from flash memory
    ip = preferences.getString("staticIP", "");  //IP address reading from flash memory

      if(ip != "")
       {
        Serial.println("ip");
        Serial.println(ip);
        convert(ip);  // seperating integers from string ip address into a, b,c,d variables
        Serial.print("Initialized IP Address: ");
        IPAddress local_IP(a, b, c, d);   //IPAddress local_IP(a, b, c, d); 
        Serial.println(local_IP); 
       }
    
    IPAddress local_IP(a, b, c, d); //converting int a, b,c,d into IP datatype
    Serial.println(ssid);
    Serial.println(password);
    if(ssid != "" && password != "") //checking ssid and password were not empty
    {
      if (!WiFi.config(local_IP, gateway, subnet))   // setting static/fixed IP address
      {      //, primaryDNS, secondaryDNS
       Serial.println("STA Failed to configure");
      }
  
      WiFi.begin(ssid.c_str(), password.c_str()); //connecting to wifi with ssid and password details
      while(WiFi.status() != WL_CONNECTED) //if wifi connection lost 
      {
        Serial.println("connecting....");
        delay(1000);
        hard_reset_button(); //hard reset button function call for reseting esp32
        retry_count++;
        if (retry_count > 10) //restarting esp32 for every 10 trys
        {
          ESP.restart();
        }
      }
      ...
      {
        MODE = 1;
        Serial.println("connected"); //printing wifi is connected and wifi name
        Serial.println(ssid);  //printing wifi name
        ...
        mqtt_client.publish(topic, "Hi, I'm ESP32 ^^"); //sending/publishing data to mqtt broker
        mqtt_client.subscribe(topic);  // subcribing/getting data from mqtt broker of topic[esp32]->intialized
      }
    }
    else
    {
      setap(); // setting softAP in esp32 if wifi not connects
    }
  }
  else
  {
...
void loop() 
{
  ...
  if (MODE)     //  retry for wifi in wifi mode
  {
    while (WiFi.status() != WL_CONNECTED) //checking wifi connection
    {
      ...
      delay(1000);
      retry_count++;
      if (retry_count > 10) // checks wifi connection for 10 times then esp32 will restrat 
      {
        ESP.restart();  
      }
    }
  }
  server_code();  //wifi server code function call 
}
