
#include <HardwareSerial.h>
#include "stdint.h"
#include <WiFi.h>

#define RXD2 16       //esp32 Rx
#define TXD2 17        //esp32 Tx

uint32_t MHeaders[] = {500, 501, 502};
volatile int i = 0, c = -1, prev = 0;                                     //reading holding 32 bit registers
byte TxData[] = {1, 3, 0x01, 0xF4, 0x00, 0x02};     //slave_id, fun code, start address, no. of bytes
uint8_t numRegs =TxData[5];                         // 1byte,     1 byte,     2 bytes ,     2 bytes
byte RxData[15];
uint16_t Maddr=502;

typedef struct arr_strc                 //creating Data stucture
{
  uint8_t id;
  uint32_t add;
  //uint16_t Data[8];
  float data[8];
}strc_arr;


volatile strc_arr dataStruct[100];

/* Table of CRC values for high-order byte */
static const uint8_t table_crc_hi[] = {
    
};

/* Table of CRC values for low-order byte */
static const uint8_t table_crc_lo[] = {
    
};

const char* network ="TP-Link_F237";
const char* password = "42191544";

WiFiServer server(80);
String header;
unsigned long currentTime = millis();    // Previous time
unsigned long previousTime = 0;   // Define timeout time in milliseconds (example: 2000ms = 2s)
const long timeoutTime = 2000;

uint16_t crc16(uint8_t *buffer, uint16_t buffer_length);
void Transmit();
//int Display_1(uint8_t *Data_rx, uint8_t i, uint8_t Regs);

void Transmit(uint16_t addr)  ///byte TxData
{
TxData[3] = byte(addr & 0xFF);
TxData[2] = byte((addr >> 8)& 0xFF);
Serial.println(TxData[2]);
Serial.println(TxData[3]);
    uint16_t  abc = crc16(TxData, 6);
  TxData[6] = byte(abc & 0xFF);
  TxData[7] = byte((abc >> 8)& 0xFF);
  //c++;
  //if(c < 5)
  //{
  digitalWrite(22, HIGH);  //transmitting
  digitalWrite(23, HIGH);

  Serial2.write(TxData, 8);
  Serial2.flush();

  digitalWrite(22, LOW);    //receiving
  digitalWrite(23, LOW);
  Maddr = addr;
  //}
}

void readRegs(uint8_t *Data_rx, uint8_t i, uint8_t Regs)      //serial printing of receiving data from available
{
  
  if(Data_rx[0] == TxData[0])
  {
    if(Data_rx[1] == TxData[1])
    {
    Regs = Data_rx[2] / 4;      
    Serial.println(Regs);        //taking from received data
    dataStruct[i].id=i;
    //Serial.print("id: ");
    Serial.println(dataStruct[i].id);
    dataStruct[i].add = Maddr;
    int regidx = 3;
    // dataStruct[i].data[0] = convertToFloat(Data_rx[3], Data_rx[4], Data_rx[5], Data_rx[6]);
    // dataStruct[i].data[1] = convertToFloat(Data_rx[7], Data_rx[8], Data_rx[9], Data_rx[10]);
    //  Serial.println(dataStruct[i].data[0]);
    //  Serial.println(dataStruct[i].data[1]);
    for(int r=0; r< Regs; r++)
    {
      //Serial.println(r+regidx);
      if(Maddr==500){ //not pf addr
        dataStruct[i].data[r] = convertToPF(Data_rx[r + regidx], Data_rx[r + (++regidx)], Data_rx[r + (++regidx)], Data_rx[r + (++regidx)]);
      }
      else{
        dataStruct[i].data[r] = convertToFloat(Data_rx[r + regidx], Data_rx[r + (++regidx)], Data_rx[r + (++regidx)], Data_rx[r + (++regidx)]);
      }
      
      Serial.println(dataStruct[i].data[r]);
    }
    // for(int  j=0; j<Regs; j++)
    // {
    // dataStruct[i].Data[j] = Data_rx[j+regidx]<<8 | Data_rx[j+(++regidx)];
    // Serial.println(dataStruct[i].Data[j]);
    // dataStruct[i].data[j] = Data_rx[j+regidx]<<24 |Data_rx[j+(++regidx)]<<16 | Data_rx[j+(++regidx)]<<8 | Data_rx[j+(++regidx)];
    // Serial.println(dataStruct[i].data[j]);
    // }
    }
  }

}

float convertToPF(uint8_t Data3, uint8_t Data4, uint8_t  Data5, uint8_t Data6 ){

  float pf;
  if(Data3){pf = Data3;}
  if(abs(Data4)){pf = Data4;}
  if(abs(Data5)){pf = -2-Data5;}
  if(Data6){pf = 2-Data6;}
  

  return pf;
}


float convertToFloat(uint8_t Data3, uint8_t Data4, uint8_t  Data5, uint8_t Data6 )
{
    uint8_t sign = Data3 & 0x80;
    uint8_t expo = (Data3 << 1) | (Data4 >> 7);
    uint32_t  m = ((Data4 & 0x7F)<<16) | (Data5 <<8) | Data6 ;
    float val = 0;

    for (int p = 22; p >= 0; --p) {
        if (m & (1 << p)) {
            val += pow(2, p - 23); // Shift by 23 because mantissa is 23 bits
        }
    }
    // if(m & 0x00400000){
    //   val += 0.5;
    // }
    // if(m & 0x00200000){
    //   val += 0.25;
    // }
    // if(m & 0x00100000){
    //   val += 0.125;
    // }
    // if(m & 0x00080000)
    // {
    //   val += (1/16);
    // }
     val+=1;
    val = val * (pow(2, expo-127));  // * (pow(-1,sign));
    if(sign)
    {
      val = -val;
    }
    Serial.println(sign);
    Serial.println(expo);
    Serial.println(m);
    //Serial.println(val);
    return val;
}

uint16_t crc16(uint8_t *buffer, uint16_t buffer_length)
{
    uint8_t crc_hi = 0xFF; /* high CRC byte initialized */
    uint8_t crc_lo = 0xFF; /* low CRC byte initialized */
    unsigned int i; /* will index into CRC lookup */

    /* pass through message buffer */
    while (buffer_length--) {
        i = crc_lo ^ *buffer++; /* calculate the CRC  */
        crc_lo = crc_hi ^ table_crc_hi[i];
        crc_hi = table_crc_lo[i];
    }

    return (crc_hi << 8 | crc_lo);
}

void setup() {
  // put your setup code here, to run once:


pinMode(22, OUTPUT);
pinMode(23, OUTPUT);

Serial2.begin(19200, SERIAL_8E1, RXD2, TXD2);
  Serial.begin(9600);
  Transmit(Maddr);
  WiFi.begin(network, password);
while(WiFi.status() != WL_CONNECTED)
{
  Serial.println("connecting...");
  delay(1000);
}
  Serial.println("connected");
   Serial.println("IP address: ");
   Serial.println(WiFi.localIP());
   
   server.begin();
   
}

void loop() 
{
  //int av = Ser
  // put your main code here, to run repeatedly:
 if(Serial2.available() > 0)
 {
  //Serial.println(Serial2.available());
  int rx_count = (numRegs*2) + 5;       //total no. of bytes to be read 
  //Serial.println(rx_count);
  Serial2.readBytes(RxData, rx_count);
  // for(int x=0; x<rx_count; x++)
  // {
  // Serial.println(RxData[x]);
  // }
  readRegs(RxData, i, numRegs);
  (i<100) ? i++ : (i=0);
  // delay(5000);
  // Transmit();
 }

 WiFiClient client = server.available(); 
 if(client)
  {
    currentTime = millis();
    previousTime = currentTime;
    Serial.println("New Client.");          // print a message out in the serial port
    String currentLine = ""; 
    while(client.connected() && currentTime - previousTime <= timeoutTime)
    {
      currentTime = millis();
      if(client.available())
      {
        char c= client.read();
        Serial.write(c);
        header +=c;
        if(c == '\n')
        {
          if(currentLine.length() == 0)
          {
            client.println("HTTP/1.1 200 OK");
            client.println("Content-type:text/html");
            client.println("Connection: close");
            client.println();         
            // Display the HTML web page
            //client.println("Hello World");
            if (header.indexOf("GET /EMDaa") >= 0) 
            {
              Serial.println("GPIO 26 on");
              //Transmit(Maddr);
              //i = 0;
            }
            else if(header.indexOf("GET /pf") >= 0){
              //Maddr = 500
              Transmit(500);
            }
            else if(header.indexOf("GET /var") >= 0){
              Transmit(502);
            }
            else if(header.indexOf("GET /curr") >= 0){
              Transmit(504);
            }
            else 
            {
              Serial.print("prev is:");
              Serial.println(prev);
              for(int b=prev; b<i; b++)
              {
                Serial.println("client");
                client.println(" {id: ");
                client.println(dataStruct[b].id);
                Serial.print("b is");
                Serial.println(b);
                client.println(",");
                client.println("Addr, ");
                client.println(dataStruct[b].add);
                client.println(", ");
                uint8_t Regs_rx = RxData[2] / 4;  //taking from received data 
                Serial.println(Regs_rx);
                for(int  j=0; j<Regs_rx; j++)
                {
                  client.print(dataStruct[b].data[j]);
                  client.print(", ");
                }
                client.println("}");
              }
              prev = i;
            }
            break;
          }
          else
          {
            currentLine ="";
          }
        }
        else if (c != '\r')
        {
          currentLine +=c;
        }
      }
    }
    header = "";
    // Close the connection
    client.stop();
    Serial.println("Client disconnected.");
    Serial.println("");
  }
}
